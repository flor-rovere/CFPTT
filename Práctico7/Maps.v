...
(* El tipo que representa al codominio de una funcion parcial. *)
...
(* Descripcion sencilla de los mappings, utilizando listas. *)
...
(* El tipo de los indices para acceder a la entrada de un map. *)
Variable index : Set.
Variable index_eq : forall x y : index, {x = y} + {x <> y}.

(* El tipo de la informacion asociada a cada indice. *)
Variable info : Set.
...
Record item : Set := 
 Item
    (* indice *)
    {item_index : index;
    (* informacion asociada al indice *)
    item_info : info 
   }.
...
(* Map apply. *)
(* Aplicacion de un map a un indice para obtener la informacion asociada
   a dicho indice. *)
